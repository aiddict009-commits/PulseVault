...
async def withdraw_handler(callback: CallbackQuery):
    ...
    ton_amount = round(user["balance"] * PULSE_TO_TON_RATE, 4)
    text = (
        f"💸 <b>Withdraw</b>\n\n"
        f"Balance: <b>{user['balance']:.4f} Pulse</b> (≈{ton_amount:.4f} TON)\n\n"
...
async def vault_buy_handler(callback: CallbackQuery):
    ...
    await callback.bot.send_invoice(
        ...
        title=f"PulseVault — Reach L{target_level} {VAULT_LEVELS[target_level]['name']}",
        description=f"Adds {stars_needed} Stars toward your vault total, unlocking L{target_level}.",
        payload=f"vault_stars:{callback.from_user.id}:{stars_needed}",
        currency="XTR",
        prices=[LabeledPrice(label="Vault Stars", amount=stars_needed)]
    )
    await callback.answer()
