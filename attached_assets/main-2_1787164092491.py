...
HOURLY_RATE = 0.15
VAULT_HOURLY_RATE = 0.30  # boosted claim rate for Vault-unlocked users
MAX_OFFLINE_HOURS = 16
MIN_WITHDRAW_FIRST = 5.0
MIN_WITHDRAW_STANDARD = 10.0
REFERRAL_BONUS = 1.0
VAULT_UNLOCK_STARS = 150  # Telegram Stars price to unlock permanent higher claim rate
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                vault_unlocked INTEGER DEFAULT 0,
                created_at INTEGER
            )
        """)
        await db.execute("""
        ...
        try:
            await db.execute("ALTER TABLE users ADD COLUMN vault_unlocked INTEGER DEFAULT 0")
            await db.commit()
        except Exception:
...
async def get_user(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT user_id, username, balance, last_claim, referral_id, total_earned, vault_unlocked FROM users WHERE user_id = ?",
            (user_id,)
        ) as cursor:
...
async def count_withdrawals(user_id: int) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM transactions WHERE user_id = ? AND type = 'withdraw' AND status != 'rejected'",
            (user_id,)
        ) as cursor:
            row = await cursor.fetchone()
            return row[0] if row else 0

async def set_vault_unlocked(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET vault_unlocked = 1 WHERE user_id = ?", (user_id,))
        await db.commit()
...
async def claim_handler(callback: CallbackQuery):
    ...
    rate = VAULT_HOURLY_RATE if user["vault_unlocked"] else HOURLY_RATE
    ...
    text = (
        ...
        + (f" • Vault rate: {VAULT_HOURLY_RATE}/h ⚡" if user["vault_unlocked"] else "")
    )
...
async def withdraw_handler(callback: CallbackQuery):
    ...
    if user["balance"] < min_required:
        if not is_first and not user["vault_unlocked"]:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"🔓 Unlock Vault ({VAULT_UNLOCK_STARS} ⭐)", callback_data="vault_unlock")],
                [InlineKeyboardButton(text="⬅️ Back to Menu", callback_data="menu")]
            ])
            text = (
                f"💸 <b>Withdraw</b>\n\n"
                f"Balance: <b>{user['balance']:.4f} Pulse</b>\n\n"
                f"The minimum withdrawal is {MIN_WITHDRAW_STANDARD:.0f} Pulse.\n\n"
                f"Unlock the Vault for {VAULT_UNLOCK_STARS} Stars to boost your claim rate "
                f"from {HOURLY_RATE}/h to {VAULT_HOURLY_RATE}/h, permanently — "
                f"you'll hit the minimum and every withdrawal after it much faster."
            )
            await callback.message.edit_text(text, reply_markup=keyboard, parse_mode=ParseMode.HTML)
        else:
            await callback.answer(f"Minimum is {min_required:.0f} Pulse", show_alert=True)
        return
...
async def vault_unlock_handler(callback: CallbackQuery):
    user = await get_user(callback.from_user.id)
    if not user:
        await callback.answer("Please /start first", show_alert=True)
        return

    if user["vault_unlocked"]:
        await callback.answer("Vault already unlocked!", show_alert=True)
        return

    await callback.bot.send_invoice(
        chat_id=callback.from_user.id,
        title="PulseVault — Unlock Vault",
        description=f"Permanently boost your claim rate to {VAULT_HOURLY_RATE}/h (from {HOURLY_RATE}/h).",
        payload=f"vault_unlock:{callback.from_user.id}",
        currency="XTR",
        prices=[LabeledPrice(label="Vault Unlock", amount=VAULT_UNLOCK_STARS)]
    )
    await callback.answer()
...
async def successful_payment_handler(message: Message):
    ...
    if payload.startswith("vault_unlock:"):
        user_id = int(payload.split(":")[1])
        await set_vault_unlocked(user_id)
        await add_transaction(user_id, "vault_unlock", VAULT_UNLOCK_STARS, note="stars payment")
        await message.answer(
            f"🔓 <b>Vault Unlocked!</b>\n\n"
            f"Your claim rate is now permanently {VAULT_HOURLY_RATE}/h.",
...
async def info_handler(callback: CallbackQuery):
    text = (
        ...
        f"• First withdrawal minimum: {MIN_WITHDRAW_FIRST:.0f} Pulse. Every withdrawal after: {MIN_WITHDRAW_STANDARD:.0f} Pulse\n"
        f"• Unlock the Vault ({VAULT_UNLOCK_STARS} Stars) to permanently boost your claim rate to {VAULT_HOURLY_RATE}/h\n\n"
...
async def main():
    ...
    dp.callback_query.register(vault_unlock_handler, F.data == "vault_unlock")
