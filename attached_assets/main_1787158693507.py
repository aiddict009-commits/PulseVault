...
async def cmd_start(message: Message):
    ...
    if not user:
        ...
        text = (
            ...
            "Earn. Claim. Get Paid. For real.\n\n"
            f"You received a welcome bonus of <b>{REFERRAL_BONUS:.2f} Pulse</b>!\n\n"
            "Your balance grows automatically while you are away.\n"
            "Come back, claim, invite friends, and withdraw when ready."
        )
    else:
        text = (
            "🔐 <b>Welcome back to PulseVault</b>\n\n"
            "Your Pulse is still accumulating.\n"
            "Tap the button below to claim or check your balance."
        )
...
async def info_handler(callback: CallbackQuery):
    text = (
        "ℹ️ <b>How it works</b>\n\n"
        f"• Earns {HOURLY_RATE} Pulse per hour\n"
        f"• Max offline: {MAX_OFFLINE_HOURS} hours\n"
        "• Claim to collect\n"
        "• Invite friends for bonus\n"
        f"• Withdraw from {MIN_WITHDRAW} Pulse"
    )
    await callback.message.edit_text(text, reply_markup=back_menu(), parse_mode=ParseMode.HTML)
    await callback.answer()
