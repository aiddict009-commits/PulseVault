...
REFERRAL_BONUS = 0.5  # new user's own welcome bonus for joining via a referral link
...
#   unlocks. Does NOT count toward the withdraw-ads gate.
...
async def stats_handler(callback: CallbackQuery):
    ...
    if user["level_expires"] > now and user["active_level"] > 1:
        ...
        active_line = "⚡ Active: L1 Starter (open — 0/h)"
    text = (
        f"📊 <b>Stats</b>\n\n"
        ...
        f"Ads watched: {user['ads_completed']}\n"
        f"Offers completed: {user['offers_completed']}\n"
...
async def tasks_handler(callback: CallbackQuery):
    ...
    text = (
        ...
        "Counts toward your withdrawal-ads requirement.\n"
        ...
        "and it's what actually funds withdrawals. Doesn't count toward your withdrawal-ads "
        "requirement."
    )
    await callback.message.edit_text(text, reply_markup=keyboard, parse_mode=ParseMode.HTML)
    await callback.answer()
